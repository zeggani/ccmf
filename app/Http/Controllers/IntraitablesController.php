...
class IntraitablesController extends Controller
{
    // public function intraitables(Request $request){
    ...
    //     $client=null;
    //   }
    //     return view('intraitables',compact("client"));
    // }
    public function intraitables(Request $request){
    if(\Auth::user()->role){
      if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
        ...
        $clients=Client::where("statutfiche","Intrbl")->where("createby",\Auth::user()->name)->get();
        ...
        $clients=Client::where( "statutfiche","Intrbl")->whereIn('createby',$users)->get();
       ...
       // si chef de QUALIF chercher ceux ajoute par les  qualif
        ...
        $clients=Client::where( "statutfiche","Intrbl")->get();
      }
      return view('intraitables',compact("clients","users"));
    }
}
}
