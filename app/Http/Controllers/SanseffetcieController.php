...
class SanseffetcieController extends Controller
{
    // public function sanseffetcie(Request $request){
    ...
    //     $client=null;
    //   }
    //     return view('sanseffetcie',compact("client"));
    // }
    public function sanseffetcie(Request $request){
      if(\Auth::user()->role){
        if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
          ...
          $clients=Client::where("statutfiche","SansEffetCie")->where("createby",\Auth::user()->name)->get();
          ...
          $clients=Client::where( "statutfiche","SansEffetCie")->whereIn('createby',$users)->get();
         ...
         // si chef de QUALIF chercher ceux ajoute par les  qualif
          ...
          $clients=Client::where( "statutfiche","SansEffetCie")->get();
        }
        return view('sanseffetcie',compact("clients","users"));
      }
