...
class SanseffetController extends Controller
{
    // public function sanseffet(Request $request){
    ...
    //     $client=null;
    //   }
    //     return view('sanseffet',compact("client"));
    // }
    public function sanseffet(Request $request){
      if(\Auth::user()->role){
        if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
          ...
          $clients=Client::where("statutfiche","SansEffet")->where("createby",\Auth::user()->name)->get();
          ...
          $clients=Client::where( "statutfiche","SansEffet")->whereIn('createby',$users)->get();
         ...
         // si chef de QUALIF chercher ceux ajoute par les  qualif
          ...
          $clients=Client::where( "statutfiche","SansEffet")->get();
        }
        return view('sanseffet',compact("clients","users"));
      }
