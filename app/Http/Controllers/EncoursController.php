...
class EncoursController extends Controller
{
    ...
    //     $client=null;
    //   }
    //     return view('encours',compact("client"));
    // }
    public function encours(Request $request){
    if(\Auth::user()->role){
      if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
        ...
        $clients=Client::where("statutfiche","EnCours")->where("createby",\Auth::user()->name)->get();
        ...
        $clients=Client::where( "statutfiche","EnCours")->whereIn('createby',$users)->get();
       ...
       // si chef de QUALIF chercher ceux ajoute par les  qualif
        ...
        $clients=Client::where( "statutfiche","EnCours")->get();
      }
      return view('encours',compact("clients","users"));
    }
}
}
