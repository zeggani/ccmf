...
class SanseffetresignesController extends Controller
{
    // public function sanseffetresignes(Request $request){
    ...
    //     $client=null;
    //   }
    //     return view('sanseffetresignes',compact("client"));
    // }
    public function sanseffetresignes(Request $request){
      if(\Auth::user()->role){
        if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
          ...
          $clients=Client::where("statutfiche","SansEffetResign")->where("createby",\Auth::user()->name)->get();
          ...
          $clients=Client::where( "statutfiche","SansEffetResign")->whereIn('createby',$users)->get();
         ...
         // si chef de QUALIF chercher ceux ajoute par les  qualif
          ...
          $clients=Client::where( "statutfiche","SansEffetResign")->get();
        }
        return view('sanseffetresignes',compact("clients","users"));
      }
