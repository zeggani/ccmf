...
class EnattenteController extends Controller
{
    ...
    //     $client=null;
    //   }
    //     return view('enattente',compact("client"));
    // }
    public function enattente(Request $request){
    if(\Auth::user()->role){
      if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
        ...
        $clients=Client::where("statutfiche","EnAtt")->where("createby",\Auth::user()->name)->get();
        ...
        $clients=Client::where( "statutfiche","EnAtt")->whereIn('createby',$users)->get();
       ...
       // si chef de QUALIF chercher ceux ajoute par les  qualif
        ...
        $clients=Client::where( "statutfiche","EnAtt")->get();
      }
      return view('enattente',compact("clients","users"));
    }
}
}
