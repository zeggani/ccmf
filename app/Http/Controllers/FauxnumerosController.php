...
class FauxnumerosController extends Controller
{
    // public function fauxnumeros(Request $request){
    ...
    //     $client=null;
    //   }
    //     return view('Fauxnumeros',compact("client"));
    // }
    public function Fauxnumeros(Request $request){
    if(\Auth::user()->role){
      if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
        ...
        $clients=Client::where("statutfiche","FxNum")->where("createby",\Auth::user()->name)->get();
        ...
        $clients=Client::where( "statutfiche","FxNum")->whereIn('createby',$users)->get();
       ...
       // si chef de QUALIF chercher ceux ajoute par les  qualif
        ...
        $clients=Client::where( "statutfiche","FxNum")->get();
      }
      return view('Fauxnumeros',compact("clients","users"));
    }
}
}
