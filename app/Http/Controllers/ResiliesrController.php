...
class ResiliesrController extends Controller
{
    // public function resiliesr(Request $request){
    ...
    //     $client=null;
    //   }
    //     return view('resiliesr',compact("client"));
    // }
    public function resiliesr(Request $request){
      if(\Auth::user()->role){
        if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
          ...
          $clients=Client::where("statutfiche","Resiliesr")->where("createby",\Auth::user()->name)->get();
          ...
          $clients=Client::where( "statutfiche","Resiliesr")->whereIn('createby',$users)->get();
         ...
         // si chef de QUALIF chercher ceux ajoute par les  qualif
          ...
          $clients=Client::where( "statutfiche","Resiliesr")->get();
        }
        return view('resiliesr',compact("clients","users"));
      }
