...
class InjoignablespermanentsController extends Controller
{
    public function injoignablespermanents(Request $request){
    if(\Auth::user()->role){
      if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
        ...
        $clients=Client::where("statutfiche","InjPer")->where("createby",\Auth::user()->name)->get();
        ...
        $clients=Client::where( "statutfiche","InjPer")->whereIn('createby',$users)->get();
       ...
       // si chef de QUALIF chercher ceux ajoute par les  qualif
        ...
        $clients=Client::where( "statutfiche","InjPer")->get();
      }
      return view('injoignablespermanents',compact("clients","users"));
    }
  }
}
