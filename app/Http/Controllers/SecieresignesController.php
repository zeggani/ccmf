...
class SecieresignesController extends Controller
{
    // public function secieresignes(Request $request){
    ...
    //     $client=null;
    //   }
    //     return view('secieresignes',compact("client"));
    // }
    public function secieresignes(Request $request){
      if(\Auth::user()->role){
        if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
          ...
          $clients=Client::where("statutfiche","SECieReSigne")->where("createby",\Auth::user()->name)->get();
          ...
          $clients=Client::where( "statutfiche","SECieReSigne")->whereIn('createby',$users)->get();
         ...
         // si chef de QUALIF chercher ceux ajoute par les  qualif
          ...
          $clients=Client::where( "statutfiche","SECieReSigne")->get();
        }
        return view('secieresignes',compact("clients","users"));
      }
