...
class InjoignablesController extends Controller
{
    ...
    //     $client=null;
    //   }
    //     return view('injoignables',compact("client"));
    // }
    public function injoignables(Request $request){
    if(\Auth::user()->role){
      if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
        ...
        $clients=Client::where("statutfiche","Inj")->where("createby",\Auth::user()->name)->get();
        ...
        $clients=Client::where( "statutfiche","Inj")->whereIn('createby',$users)->get();
       ...
       // si chef de QUALIF chercher ceux ajoute par les  qualif
        ...
        $clients=Client::where( "statutfiche","Inj")->get();
      }
      return view('injoignables',compact("clients","users"));
    }
}
}
