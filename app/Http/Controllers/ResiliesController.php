...
class ResiliesController extends Controller
{
    // public function resilies(Request $request){
    ...
    //     $client=null;
    //   }
    //     return view('resilies',compact("client"));
    // }
    public function resilies(Request $request){
      if(\Auth::user()->role){
        if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
          ...
          $clients=Client::where("statutfiche","Resilies")->where("createby",\Auth::user()->name)->get();
          ...
          $clients=Client::where( "statutfiche","Resilies")->whereIn('createby',$users)->get();
         ...
         // si chef de QUALIF chercher ceux ajoute par les  qualif
          ...
          $clients=Client::where( "statutfiche","Resilies")->get();
        }
        return view('resilies',compact("clients","users"));
      }
