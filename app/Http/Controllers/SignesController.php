...
class SignesController extends Controller
{
    ...
    //     $client=null;
    //   }
    //     return view('signes',compact("client"));
    // }
    public function signes(Request $request){
      if(\Auth::user()->role){
        if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
          ...
          $clients=Client::where("statutfiche","Sign")->where("createby",\Auth::user()->name)->get();
          ...
          $clients=Client::where( "statutfiche","Sign")->whereIn('createby',$users)->get();
         ...
         // si chef de QUALIF chercher ceux ajoute par les  qualif
          ...
          $clients=Client::where( "statutfiche","Sign")->get();
        }
        return view('signes',compact("clients","users"));
      }
