...
class PromessesController extends Controller
{
    // public function promesses(Request $request){
    ...
    //     $client=null;
    //   }
    //     return view('promesses',compact("client"));
    // }
    public function promesses(Request $request){
    if(\Auth::user()->role){
      if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
        ...
        $clients=Client::where("statutfiche","Prom")->where("createby",\Auth::user()->name)->get();
        ...
        $clients=Client::where( "statutfiche","Prom")->whereIn('createby',$users)->get();
       ...
       // si chef de QUALIF chercher ceux ajoute par les  qualif
        ...
        $clients=Client::where( "statutfiche","Prom")->get();
      }
      return view('promesses',compact("clients","users"));
    }
}
}
