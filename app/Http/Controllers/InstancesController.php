...
class InstancesController extends Controller
{
    ...
    if(\Auth::user()->role){
      if(\Auth::user()->role->name=="OPERATEURS" || \Auth::user()->role->name=="QUALIF" ){
        ...
        $clients=Client::where("statutfiche","Inst")->where("createby",\Auth::user()->name)->get();
        ...
        $clients=Client::where( "statutfiche","Inst")->whereIn('createby',$users)->get();
       ...
       // si chef de QUALIF chercher ceux ajoute par les  qualif
        ...
        $clients=Client::where( "statutfiche","Inst")->get();
      }
      return view('instances',compact("clients","users"));
    }
}
}
